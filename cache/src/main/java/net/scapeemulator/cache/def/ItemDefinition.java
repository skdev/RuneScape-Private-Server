...
public final class ItemDefinition {
	...
	private int highAlchValue, lowAlchValue;
	private int slot;
	private double speed;
	
	private int[] bonuses = new int[14];
	
	public void setItemBonuses(int... bonus) {
		for (int i = 0; i < bonuses.length; i++) {
			bonuses[i] = bonus[i];
		}
	}
	public int getAttackBonus(ItemBonus bonus) {
		if (bonus == ItemBonus.STAB) {
			return bonuses[0];
		}
		if (bonus == ItemBonus.SLASH) {
			return bonuses[1];
		}
		if (bonus == ItemBonus.CRUSH) {
			return bonuses[2];			
		}
		if (bonus == ItemBonus.MAGIC) {
			return bonuses[3];
		}
		if (bonus == ItemBonus.RANGED) {
			return bonuses[4];	
		}
		return 0;
	}

	public int getDefenceBonus(ItemBonus bonus) {
		if (bonus == ItemBonus.STAB) {
			return bonuses[5];
		}
		if (bonus == ItemBonus.SLASH) {
	...
	public int getOtherBonus(ItemBonus bonus) {
		if (bonus == ItemBonus.STRENGTH) {
			return bonuses[10];
		}
		if (bonus == ItemBonus.RANGED_STRENGTH) {
			return bonuses[11];
		}
		if (bonus == ItemBonus.MAGIC_STRENGTH) {
			return bonuses[12];	
		}
		if (bonus == ItemBonus.PRAYER) {
			return bonuses[13];
		}
		return 0;
	}
