	...
	public static boolean consume(Player player, int slot) {
		...
		if (item != null) {
			...
			if (canEat) {
				...
				if (amt-- != 0 && item.getDefinition().isStackable()) {
					player.getInventory().add(new Item(item.getId(), (amt - 1)), slot);
				}
				
				int level = player.getSkillSet().getLevel(Skill.HITPOINTS);
				int currentLevel = player.getSkillSet().getCurrentLevel(Skill.HITPOINTS);
				int heal = currentLevel + food.getHealth();
				
				player.getSkillSet().setCurrentLevel(Skill.HITPOINTS, (heal > level ? level : heal));
