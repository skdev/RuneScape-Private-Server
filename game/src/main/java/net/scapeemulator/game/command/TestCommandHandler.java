...
public class TestCommandHandler extends CommandHandler {
	...
	//	player.getInterfaceSet().openCharDesign();
		
		//.arguments//player.getTrade().sendRequest(World.getWorld().getPlayerByName(arguments[0]));
		
		player.send(new UpdateFriendMessage(player, arguments[0]));
		
	//	player.send(new TestMessage());	
		
	//	player.send(new MusicMessage(Integer.parseInt(arguments[0]), Integer.parseInt(arguments[1])));
		/*int npcs = player.getLocalNpcs().size();
		
		for (int i = 0; i <= npcs; i++)
			player.send(new NpcAnimationMessage(i, 866, 0));*/
		
	//	int id = Integer.parseInt(arguments[0]);
		//player.getInterfaceSet().openChatBoxInterface(id);
		
	//	player.send(new FriendsStatusMessage(2));
		
		
		//player.send(new ConfigMessage(563, 41946304));
		//player.send(new ConfigMessage(1248, -2013265920));
		
		//Player p = World.getWorld().getPlayerByName(arguments[0]);
		//int status = Integer.parseInt(arguments[0]);
		//player.send(new MapBlackoutMessage(status));
		
		//player.getSkillSet().setCurrentLevel(Skill.HITPOINTS, 30);
		//player.getCombat().attack(player);

		
		//player.setHitUpdated(true);
		
		//player.send(new ServerMessage("<shad=2><trans=50><col=493>Hello World"));
	//	player.getInterfaceSet().openWindow(667); // 334
		//14 music name
	//	for (int i = 0; i <= Integer.parseInt(arguments[0]); i++) {
		//	player.send(new InterfaceTextMessage(187, i, "" + i));
		//}
